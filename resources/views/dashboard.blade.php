...
<div class="space-y-6">
    ...
    @if (auth()->check() && ((int) auth()->user()->department_id === 2 || (int) auth()->user()->department_id === 4 || str_contains(strtolower(auth()->user()->department?->name ?? ''), 'hr')))
        <div class="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 rounded-2xl p-5 sm:p-6 text-white shadow-xl border border-blue-700/50 flex flex-col md:flex-row items-center justify-between gap-5 relative overflow-hidden">
            <div class="flex items-center gap-3.5">
                <div class="w-12 h-12 rounded-xl bg-blue-600/40 border border-blue-400/40 flex items-center justify-center shrink-0 shadow-md">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <h2 class="text-sm sm:text-base font-black text-white tracking-tight">{{ __('Akses Khusus Departemen HRD: Master Data & Bulk Import CSV') }}</h2>
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-emerald-500/20 text-emerald-300 border border-emerald-400/30">{{ __('The Vault') }}</span>
                    </div>
                    <p class="text-xs text-blue-200 mt-1 max-w-2xl leading-relaxed">
                        {{ __('Kelola data resmi karyawan, tambah manual karyawan baru, serta lakukan Bulk Upload CSV untuk sinkronisasi NIK/KTP pendaftaran akun seluruh divisi.') }}
                    </p>
                </div>
            </div>

            <a href="{{ url('/hcm-core/employees-master') }}" 
               class="shrink-0 w-full md:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 active:scale-95 text-white font-bold text-xs uppercase tracking-wider shadow-lg shadow-blue-900/40 border border-blue-400/30 transition cursor-pointer">
                <span>{{ __('Buka Menu Import CSV & Master Data') }}</span>
