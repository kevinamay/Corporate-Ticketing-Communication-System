<div class="relative" x-data="{ open: false }">
        ...
        <div x-show="open" @click.away="open = false" 
            ...
            <div class="p-3 space-y-2 text-xs bg-slate-50 dark:bg-slate-800/60 border-b border-gray-100 dark:border-slate-800">
                <div class="flex items-center justify-between text-[11px]">
                    <span class="text-slate-500 dark:text-slate-400">{{ __('Status Akun:') }}</span>
                ...
                @if ($currentUser->national_id_ktp)
                    <div class="flex items-center justify-between text-[11px]">
                        <span class="text-slate-500 dark:text-slate-400">{{ __('Nomor KTP (NIK):') }}</span>
                ...
                @if ($currentUser->whatsapp_number)
                    <div class="flex items-center justify-between text-[11px]">
                        <span class="text-slate-500 dark:text-slate-400">{{ __('WhatsApp:') }}</span>
                ...
                @if ($currentUser->complete_address)
                    <div class="pt-1 border-t border-slate-200 dark:border-slate-700 text-[11px]">
                        <span class="text-slate-500 dark:text-slate-400 block mb-0.5">{{ __('Alamat:') }}</span>
            ...
            <div class="p-2 bg-white dark:bg-slate-900">
                <form action="{{ route('logout') }}" method="POST" class="w-full">
                    ...
                    <button type="submit" class="w-full py-2 px-3 rounded-lg text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 font-semibold transition text-xs flex items-center justify-between cursor-pointer">
                        <span>{{ __('Keluar Akun (Logout)') }}</span>
    ...
    @else
        ...
        <button @click="open = !open" type="button" 
            ...
            <div class="text-left hidden sm:block">
                <div class="flex items-center gap-1.5">
                    <p class="text-xs font-semibold text-white leading-tight">{{ __('Belum Masuk') }}</p>
                    <span class="text-[9px] uppercase font-bold px-1.5 py-0.2 rounded bg-amber-500/30 text-amber-200 border border-amber-400/30">
                        {{ __('TAMU') }}
                    </span>
                </div>
                <p class="text-[10px] text-blue-200 leading-tight">{{ __('Klik untuk Login / Daftar') }}</p>
            </div>
            <svg class="w-3.5 h-3.5 text-blue-200 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
        ...
        <div x-show="open" @click.away="open = false" 
            ...
            <div class="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-4 text-white">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h4 class="text-xs font-bold text-white">{{ __('Portal Layanan Tiket') }}</h4>
            ...
            <div class="p-4 text-xs text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/60 border-b border-gray-100 dark:border-slate-800 leading-relaxed">
                {{ __('Anda saat ini belum masuk ke akun. Silakan login atau daftarkan akun baru untuk mengirim tiket dan mengakses komunikasi tim.') }}
            </div>
            ...
            <div class="p-3 bg-white dark:bg-slate-900 space-y-2">
                <a href="{{ route('login') }}" 
                   class="w-full py-2.5 px-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition">
                    ...
                    <span>{{ __('Masuk ke Akun (Login)') }}</span>
                </a>
