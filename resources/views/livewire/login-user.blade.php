    ...
    <div class="relative z-10 w-full max-w-md bg-white shadow-2xl border border-gray-100 rounded-xl overflow-hidden my-8">
        ...
        <div class="p-6 sm:p-8">
            <div class="mb-6 text-center">
                ...
                <p class="text-xs text-slate-500 mt-1">Masuk dengan Nomor KTP atau Alamat Email Perusahaan Anda.</p>
            </div>

            @if (session('status'))
                <div class="mb-5 p-3.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs flex items-center gap-2">
            ...
            @if ($errorMessage)
                <div class="mb-5 p-3.5 rounded-lg bg-rose-50 border border-rose-200 text-rose-700 text-xs flex items-center gap-2">
            ...
            <form wire:submit.prevent="login" class="space-y-4">
                ...
                <!-- Input: KTP or Email -->
                <div>
                    <label for="login_id" class="block text-xs font-bold text-slate-700 mb-1">
                        Nomor KTP atau Alamat Email <span class="text-rose-500">*</span>
                    </label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                        </div>
                        <input type="text" id="login_id" wire:model="login_id" placeholder="16 digit KTP atau email@perusahaan.com"
                               class="w-full text-xs pl-9 pr-3.5 py-2.5 rounded-lg border @error('login_id') border-rose-400 bg-rose-50 @else border-gray-300 @enderror focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition">
                    </div>
                    @error('login_id') <p class="text-[11px] text-rose-600 mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <a href="#" class="text-[11px] text-blue-600 hover:text-blue-800 transition">Lupa password?</a>
                    </div>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                        ...
                        <input type="password" id="password" wire:model="password" placeholder="Masukkan password Anda"
                               class="w-full text-xs pl-9 pr-3.5 py-2.5 rounded-lg border @error('password') border-rose-400 bg-rose-50 @else border-gray-300 @enderror focus:border-blue-600 focus:ring-2 focus:ring-blue-100 outline-none transition">
                    </div>
            ...
            <!-- Link to Register -->
            <div class="mt-6 pt-5 border-t border-slate-200 text-center text-xs text-slate-600">
                Belum memiliki akun?
                <a href="{{ route('register') }}" class="font-bold text-blue-600 hover:text-blue-800 transition underline ml-1">
                    Daftar Akun Baru
                </a>
            </div>
